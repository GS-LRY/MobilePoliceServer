...
function doPost(o,msg,link){
	var form = $(o).closest('form');
	var url = form.attr('action');
	$.post(url, form.serialize(),function(r){
		if(r.status=='ok'){
			showMessage(msg,link);
		}else{
			art.dialog.alert(r.msg);
		}
	});
}
function jsonDoPost(o){
	var form = $(o).closest('form');
	var url = form.attr('action');
	$.post(url, form.serialize(),function(r){
        var data = r;
